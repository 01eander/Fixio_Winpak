...
import { Search, Clock, Plus, X, Save, Edit, Trash2, ArrowLeft } from 'lucide-react';
...
export default function Shifts() {
    ...
    const [formData, setFormData] = useState({ name: '', description: '', daily_hours: '' });
    ...
    const handleOpenModal = (item = null) => {
        if (item) {
            ...
            setEditingItem(null);
            setFormData({ name: '', description: '', daily_hours: '' });
        }
        setIsModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const url = editingItem ? `${API_URL}/shifts/${editingItem.id}` : `${API_URL}/shifts`;
    ...
    const filteredItems = shifts.filter(item =>
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-2xl font-bold text-gradient">Turnos (Shifts)</h2>
                </div>
                <button
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-[var(--border-glass)] bg-white/5 text-[var(--text-muted)] text-sm uppercase">
                            ...
                            <th className="p-4 font-semibold text-center">Horas Diarias</th>
                        ...
                        {loading ? (
                            ...
                            filteredItems.map((item) => (
                                <tr key={item.id} className="hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="p-4 text-[var(--text-muted)]">{item.description || '-'}</td>
                                    <td className="p-4 text-[var(--text-main)] text-center font-mono">
                                        {parseFloat(item.daily_hours).toFixed(2)} hrs
                                    </td>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-[var(--bg-panel)] border border-[var(--border-glass)] rounded-xl w-full max-w-sm shadow-2xl p-6">
                        <div className="flex justify-between items-center mb-6 border-b border-[var(--border-glass)] pb-4">
                            <h3 className="text-xl font-bold text-white">{editingItem ? 'Editar Turno' : 'Nuevo Turno'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-[var(--text-muted)] mb-1">Horas Diarias</label>
                                <input
                                    type="number"
                                    step="0.5"
                                    name="daily_hours"
